/**
 * Cloudflare Pages Function — POST /api/plan/interpret
 *
 * Production BFF. 로직은 `server/planInterpretRoute.ts`(런타임 무관, Vite 미들웨어와 공유)에
 * 있다. API 키는 `context.env`(Cloudflare Pages 서버 환경변수)에서만 읽는다.
 */
import { handlePlanInterpretRoute } from "../../../server/planInterpretRoute";
...
const ROUTE = "POST /api/plan/interpret";
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  const result = await handlePlanInterpretRoute(
    body,
