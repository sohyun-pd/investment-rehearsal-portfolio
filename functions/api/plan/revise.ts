/**
 * Cloudflare Pages Function — POST /api/plan/revise
 *
 * Production BFF. 로직은 `server/planReviseRoute.ts`(런타임 무관, Vite 미들웨어와 공유)에 있다.
 * API 키는 `context.env`(Cloudflare Pages 서버 환경변수)에서만 읽는다.
 */
import { handlePlanReviseRoute } from "../../../server/planReviseRoute";
...
const ROUTE = "POST /api/plan/revise";
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  const result = await handlePlanReviseRoute(
    body,
